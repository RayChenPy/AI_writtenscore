...
# model_path = './Flask_t1/models/basic_model-best-model.h5'
...
def model_predict(img_path, fileName):#, model
    #print(model.summary())
    # Preprocessing the image#data = [[img_path]]
    data = {'id':[img_path],'class':[""]}
    # df = pd.DataFrame(data)
    # data_test_generator_1 = data_test_datagen.flow_from_dataframe(
    #                                                 dataframe=df,
    #                                                 #directory=img_path,
    #                                                 x_col="id",
    #                                                 target_size=(112,112),
    ...
    #                                                 class_mode=None,
    #                                                 shuffle=False)
    # name_list,number_list=pretreatment(img_path)   #預處理function,回傳兩個list,第一個是檔名list,第二個是算成績list
    # x_test=get_testdata(name_list)                 #矩陣化餵模型
    # x_test = preprocess_input(x_test)
    ...
    #Ray新增
    predict_final,predict_score,predict_score_final,predict_score_mean,dis_count,,size_count,error = predict(img_path)
#        predict_final,predict_score,predict_score_final,predict_score_mean,dis_count,size_count,error=predict(img_path)
    #Ray新增
    ...
    Allstring=dict()
    Allstring['s1'] = f'原始得分:{predict_score_mean}'
    Allstring['s2']  = f'你的平均得分為:{((int(predict_score_mean)/8)**0.5)*100:.0f}'
    ...
    # re=[]
    # for n in num:
    #     re.append(numMap[numMap['class']==n]['id'].values[0])
    # print(re)
    # str1 = ','.join(re)
    # str1="1"
    return Allstring
