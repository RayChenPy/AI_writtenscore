...
from tensorflow.keras.layers import (Dense, Dropout, Activation,Flatten, GlobalAveragePooling2D,GlobalMaxPooling2D)
...
numMap = pd.read_csv('./models/num_class_map_3.csv')
...
def score(y_score,y_pred):
    ...
    return y_pred_score_final,error,error_character
#Ray新增
def predict(book):
    ...
    model_resnet.load_weights('./models/basic_model-best-model-character2.h5')
...
#             for z , each in enumerate(img2):
#                 if each == 255:
#                     img2[z] = random.randint(50,205)
#             img3=img2.reshape((img.shape[0],img.shape[1],-1))
...
#------------------------------------------------------------------
    ...
    model_score = Model(input_tensor, output)
#------------------------------------------------------------------------------
   # x_score = GlobalAveragePooling2D()(pre_model_score.output)
   # outputs_score = Dense(1, activation='relu')(x_score)
    #model_score = Model(inputs=pre_model_score.inputs, outputs=outputs_score)
    model_score.load_weights('./models/best_model_0625_112.h5')
    y_pred_score = model_score.predict(x_test_score)
    
    y_pred_score_final,error,error_character=score(y_pred_score,y_pred)
...
#             y_pred_score_final.append(float(each))
    ...
    print(f'在這裡啦....{dis_character}')
    size_mean,size_count,size_character=size(number_list) #寫太小
    print(f'在這裡啦....{size_character}')
    #return y_pred,y_pred_score,y_pred_score_final,y_pred_score_mean,dis_count,size_count,error
    return y_pred,y_pred_score,y_pred_score_final,y_pred_score_mean,dis_count,size_count,error,number_list,error_character
# def error_list(predict):
#     error_list = []
#     for number,i in enumerate(predict):
#         for each in i[1:]:
#             if each != ans[number]:
...
#     return error_list
def getScore(img_path,number_list,error_character,AA):
    
    if AA=="甲上":
        evaluate = Image.open('./eval/score_2.jpg')
    elif AA=="甲":
        evaluate = Image.open('./eval/score_2.jpg')
    elif AA=="甲下":
        evaluate = Image.open('./eval/score_2.jpg')
    ...
    evaluateNew = evaluate.resize((150, 150))
    #evaluateNew = evaluate.resize(evaluate, (20, 20))
    dis_mean,dis_count,dis_character=displacement(number_list) #寫偏
    size_mean,size_count,size_character=size(number_list) #寫太小
    ...
    error_list_axis = []
    for i in error_character:
        error_list_axis.append(d[i])
    img_icon = add_icon(img_path,dis_character_axis,size_character_axis,error_list_axis)
    # 寫入圖檔
    img_icon.paste(evaluateNew, (900,600))
    
    #img_icon[0:evaluateNew.shape[0], 0:evaluateNew.shape[1]] = evaluateNew
    img_icon.save('./static/result/output.jpg')
...
def model_predict(img_path, fileName):
    ...
    #Ray新增
    predict_final,predict_score,predict_score_final,predict_score_mean,dis_count,size_count,error,number_list,error_character=predict(img_path)
    #Ray新增
    ...
    Allstring['s1'] = f'你的平均得分為:{AA}'#f'原始得分:{SS}'
    Allstring['s2']  = f'你有{error}個字寫錯囉'
    #print(f'dis_mean{dis_mean}')
    Allstring['s3']  = f'你有{dis_count}個字寫偏囉'
    Allstring['s4']  = f'你有{size_count}個字寫太小囉'
    #print(f'dis_character{dis_character}')
    #print(f'size_mean{size_mean}')
    Allstring['s5']  = f'X:寫錯了 / 藍色框:寫太小 / 紅色框:寫偏 '#f'你有{size_count}個字寫太小囉'
    ...
    getScore(img_path,number_list,error_character, AA)

    print(check)
    return Allstring
def add_icon(img_path, list_1, list_2, list_3):
    img = Image.open(img_path) #生字簿
    img = img.resize((1240,1755))
    ring = Image.open('./picture/ring_red.png')
    frame = Image.open('./picture/frame_blue.png')
    cross = Image.open('./picture/cross_orange.png')
    print('list_1:',list_1)
    print('list_2:',list_2)
    for i in list_1:  
        x,y,w,h = i
    ...
    for i in list_3:  
        x,y,h,w = i
        location = [x,y,x+h,y+w]
        cross = cross.resize((h,w))
        img.paste(cross,location,cross) #第三個參數把透明部分顯示出來
    return img
